...
public class SleepProgressbarView extends View {
	...
	public SleepProgressbarView(Context context, AttributeSet attrs) {
		super(context, attrs);
		// 获取自定义的属性
	...
	private void initVariable() {
		...
		mCirclePaint.setStrokeWidth(mStrokeWidth/4);
		...
		if (mProgress > 0 ) {
			...
			if(mProgress<=25){
				...
				y = mYCenter -mRadius * Math.cos(((float) mProgress / 100) * 2 * Math.PI);
				 ...
				 x = mXCenter+mRadius * Math.sin(((float)mProgress /100) *2* Math.PI);
