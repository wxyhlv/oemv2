package com.capitalbio.oemv2.myapplication.View.calendar;
...
public class CalendarView extends View implements View.OnTouchListener {
	...
	private void drawCellText(Canvas canvas, int index, String text, int color) {
		...
		float cellX = (surface.cellWidth * (x - 1))
				...
				/ 2f;
		float cellYi = surface.monthHeight + surface.weekHeight + (y - 1)
				* surface.cellHeight + surface.cellHeight;
		canvas.drawText(text, cellX, cellY, surface.datePaint);
		//canvas.drawText(".", cellX + 8, cellYi, surface.datePaint);
	}
	 ...
	 * @param color
	 */
	private void drawCellBg_round(Canvas canvas, int index, int color) {
		int x = getXByIndex(index);
		int y = getYByIndex(index);
		surface.cellBgPaint.setColor(color);
		float left = surface.cellWidth * (x - 1) + surface.borderWidth;
	...
	private void drawDownOrSelectedBg(Canvas canvas) {
		// down and not up
		if (downDate != null) {
			drawCellBg_round(canvas, downIndex, surface.cellDownColor);
		}
		// selected bg color
		if (!selectedEndDate.before(showFirstDate)
			...
			for (int i = section[0]; i <= section[1]; i++) {
				drawCellBg_round(canvas, i, surface.cellSelectedColor);
			}
		}
	}
