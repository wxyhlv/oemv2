...
public class TasksCompletedView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mProgress > 0 ) {
			...
			double x = mXCenter+(mRadius+mTxtHeight) * Math.sin(((float)mProgress /100) * 360 * Math.PI /180);

			double y = mYCenter -(mRadius+mTxtHeight)*Math.cos(((float)mProgress /100) * 360* Math.PI /180);
