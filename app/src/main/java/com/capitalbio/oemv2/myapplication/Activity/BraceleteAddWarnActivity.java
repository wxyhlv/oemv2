...
public class BraceleteAddWarnActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initChildView() {
        ...
        if(!ismodify){
            ...
            setTvTopTitle(R.string.modifyWarn);
            String type = getIntent().getStringExtra("warntype");
            String warntime = getIntent().getStringExtra("warntime");
            String warnrepeat = getIntent().getStringExtra("warnrepeat");
            String warnmemo = getIntent().getStringExtra("warnmemo");
            OemLog.log(TAG,"REPEAT " + warnrepeat);
            OemLog.log(TAG,"warnmemo " + warnmemo);
            tv_warntype.setText(type);
            tv_warnTime.setText(warntime);
            if(TextUtils.isEmpty(warnrepeat)){
                warnrepeat = "永不";
            }
            tv_repeat_select.setText(warnrepeat);
            memoEdt.setText(warnmemo);
        }
    }
